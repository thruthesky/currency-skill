...
Future<void> main() async {
  ...
  try {
    ...
    final usdRates = await fetchJson('$baseUrl/latest?base=USD&symbols=KRW,PHP');
    printJson(usdRates);

    // 3. 100 USD를 KRW로 변환
    printSeparator('3. Convert 100 USD to KRW');
    final converted = await fetchJson('$baseUrl/latest?base=USD&symbols=KRW&amount=100');
    printJson(converted);
    ...
    final historical = await fetchJson('$baseUrl/2024-01-15?base=USD&symbols=KRW');
    ...
    final phpRates = await fetchJson('$baseUrl/latest?base=PHP&symbols=KRW,USD');
    printJson(phpRates);

    // 7. KRW 기준 PHP, USD 환율
    printSeparator('7. KRW -> PHP, USD');
    final krwRates = await fetchJson('$baseUrl/latest?base=KRW&symbols=PHP,USD');
    printJson(krwRates);
